...
func (handler *handler) ListBlogCategory() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		request := presenter.BlogCategoryListReq{
			Page:     int(utils.CheckPageInQuery(c)),
			PageSize: int(utils.CheckPageSizeInQuery(c)),
		}

		category, totalPage, err := handler.usecase.ListBlogCategory(request)
		if err != nil {
